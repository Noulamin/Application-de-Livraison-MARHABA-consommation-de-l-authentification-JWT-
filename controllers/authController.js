...
exports.signup = (req, res) => {
    ...
    user.save((err, user) => {
        ...
        transporter.sendMail({
            ...
            html: `<p>Click <a href="http://localhost:2002/api/auth/emailVerification/${token}">Here</a> to verify your email address.</p>`
        }).then(e => {
            user.hashed_password = undefined
            user.salt = undefined
            return res.status(200).json({ user, message: 'An email has been sent to your email for verification' })
        })
    })
}
    ...
    User.findOne({ email }, (err, user) => {
        if (err || !user)
            return res.status(400).json({
                erreur: 'User not found with this email.'
            })
        if (user.emailIsVerified == false)
            return res.status(401).json({ erreur: "Email is not verified, Please check your email." })
